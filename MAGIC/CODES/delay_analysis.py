...
command = "ngspice adderdestination.ckt"
...
for j in range(0,8): #j represents the iteration of input. since we have 8 inputs we have kept it as 0,8
    ...
    for i in range(0,4):  #i is for output sequence
        ...
        if(j<4):
            fp1 = open("addersub.ckt",'r') #for input sequence 1
        else:
            fp1 = open("adddersub.ckt",'r') #for input sequecne 2
        #if you have any other input sequences you can add

        fp2 =open("adderdestination.ckt",'w') #destination file for running
